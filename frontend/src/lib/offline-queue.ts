...
export function pendingAnswers(storage:Storage) {
  ...
  for(let index=0;index<storage.length;index++){
    ...
    try{answers.push(JSON.parse(storage.getItem(key)!) as PendingAnswer);}catch{storage.removeItem(key);}
  }
  return answers;
}
export async function flushAnswers(storage:Storage,send:(answer:PendingAnswer)=>Promise<void>) {
  for(const answer of pendingAnswers(storage)){
    try{await send(answer);storage.removeItem(`${PREFIX}${answer.submissionId}:${answer.questionId}`);}catch{/* Online event retries later. */}
  }
}
