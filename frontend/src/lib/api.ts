...
export type LiveChallengeStatus='PUBLISHED'|'LOBBY'|'QUESTION_ACTIVE'|'ANSWERS_LOCKED'|'PEER_MARKING'|'ROUND_RESULTS'|'FINISHED'|'PAUSED'|'CANCELLED';
export interface LiveChallengeStudentCard {id:string;title:string;classId:string;className:string;status:Exclude<LiveChallengeStatus,'FINISHED'|'CANCELLED'>;teacherName:string;syllabusCode:string;topicTitle:string|null;subtopicTitle:string|null;questionCount:number;joinedCount:number;participantStatus:'JOINED'|'LEFT'|'REMOVED'|null;joinedAt:string|null;publishedAt:string|null;startedAt:string|null;currentQuestionPosition:number|null;canJoin:boolean}
export interface LiveChallengeJoinResult {id:string;title:string;classId:string;className:string;status:string;teacherName:string;joinedCount:number;joined:true}
export interface LiveChallengeRoundState {id:string|null;number:number|null;status:string|null;startedAt:string|null;lockedAt:string|null;markingStartedAt:string|null;resultsReleasedAt:string|null;timeLimitSeconds:number|null}
export interface LiveChallengeState {id:string;title:string;classId:string;className:string;syllabusCode:string;topicTitle:string|null;subtopicTitle:string|null;status:LiveChallengeStatus;stateVersion:number;currentQuestionPosition:number|null;serverNow:string;round:LiveChallengeRoundState|null;question:AttemptQuestion|null;markScheme:Record<string,unknown>|null;source:Record<string,unknown>|null}
