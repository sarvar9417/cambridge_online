...
export interface LiveChallengeStudentCard {id:string;title:string;classId:string;className:string;status:'PUBLISHED'|'LOBBY'|'QUESTION_ACTIVE'|'ANSWERS_LOCKED'|'PEER_MARKING'|'ROUND_RESULTS'|'PAUSED';teacherName:string;syllabusCode:string;topicTitle:string|null;subtopicTitle:string|null;questionCount:number;joinedCount:number;participantStatus:'JOINED'|'LEFT'|'REMOVED'|null;joinedAt:string|null;publishedAt:string|null;startedAt:string|null;currentQuestionPosition:number|null;canJoin:boolean}
