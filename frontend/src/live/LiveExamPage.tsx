...
function LiveQuestionView({question,audience='student'}:{question:LiveExamQuestion;audience?:'student'|'staff'}) {
  ...
  return <article className="live-question-card">
    ...
    {question.dependencyWork.length?<section className="live-dependency-work">
      ...
      {question.dependencyWork.map((dependency)=><article key={dependency.questionId}>
        ...
        {dependency.ownAnswer!==null?<pre>{dependency.ownAnswer||'Javob bo‘sh topshirilgan.'}</pre>:<p>{dependency.position===null?'Bu majburiy qism sessiyada topilmadi.':audience==='staff'?'O‘quvchi bu yerda o‘zining oldingi javobidan foydalanadi.':'Oldingi javob mavjud emas — siz bu qism bajarilgandan keyin qo‘shilgan bo‘lishingiz mumkin.'}</p>}
      </article>)}
    </section>:null}
  ...
  return <div className="live-page live-landing">
    ...
    {user.role==='student'?<section className="live-join-card">
      ...
      <section className="live-create-main"><span className="live-step">1</span><div><h2>Sessiya</h2><p>Sinf va savollar ko‘lamini tanlang.</p></div>
        ...
        <div className="live-topic-grid"><fieldset><legend>Topic</legend>{topics.map((topic)=><label key={topic.topic_id}><input type="checkbox" checked={topicIds.includes(topic.topic_id)} onChange={()=>{
              const nextTopics=topicIds.includes(topic.topic_id)
                ? topicIds.filter((id)=>id!==topic.topic_id)
                : [...topicIds,topic.topic_id];
              setTopicIds(nextTopics);
              setSubtopicIds((current)=>nextTopics.length
                ? current.filter((id)=>syllabusTopics.some((row)=>row.subtopic_id===id&&nextTopics.includes(row.topic_id)))
                : current);
              setQuestionPool([]);setSelectedQuestionIds([]);
            }}/><span>{topic.topic_number}. {topic.topic_title}</span></label>)}</fieldset>
...
function ProjectorView({snapshot,sessionId}:{snapshot:LiveExamBoardSnapshot;sessionId:string}) {
  ...
  return <div className="live-projector-overlay">
    ...
    <main className={session.pausedAt?'is-paused':''}>
      ...
      {session.status==='question_open'&&snapshot.question?<><LiveQuestionView question={snapshot.question} audience="staff"/><div className="live-projector-count"><UsersThree size={32}/><strong>{session.submittedCount}/{session.participantCount}</strong><span>javob topshirdi</span></div></>:null}
