...
describe('Live Challenge final frontend experience',()=>{
  ...
  it('reflects the teacher-override policy in the runtime instead of relying on an API rejection',()=>{
    expect(runtime).toContain('snapshot.session.teacherOverrideEnabled');
    expect(runtime).toContain('Teacher override bu challenge uchun o‘chirilgan.');
    expect(runtime).toContain("snapshot.session.teacherOverrideEnabled?'Audit bilan bahoni yangilash':'Override o‘chirilgan'");
  });
});
