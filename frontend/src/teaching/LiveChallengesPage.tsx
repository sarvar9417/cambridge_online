...
export function LiveChallengesPage(){
  ...
  return <main className="live-page">
    ...
    {lobby?<section className="live-lobby">
      ...
      <div className="live-lobby-actions"><button type="button" className="secondary" onClick={()=>void viewLobby(lobby.id)} disabled={saving}>Yangilash</button><button type="button" className="secondary" onClick={()=>openBoard(lobby.id)}>Board</button><button type="button" className="secondary" onClick={()=>setLobby(null)}>Yopish</button><button type="button" className="live-primary" disabled={saving||lobby.status!=='LOBBY'||lobby.participantCount<1} onClick={()=>void startChallenge(lobby)}>{saving?'Boshlanmoqda…':'Start challenge'}</button></div>
    </section>:null}
...
function TeacherRound({state,busy,onRefresh,onBoard,onLock,onStartPeer,onRelease,onNext,onPause,onResume,onCancel,onClose}:{state:LiveChallengeState;busy:boolean;onRefresh:()=>Promise<void>;onBoard:()=>void;onLock:()=>Promise<void>;onStartPeer:()=>Promise<void>;onRelease:()=>Promise<void>;onNext:()=>Promise<void>;onPause:()=>Promise<void>;onResume:()=>Promise<void>;onCancel:()=>Promise<void>;onClose:()=>void}){
  ...
  return <section className="live-round-panel">
    ...
    {['ROUND_RESULTS','FINISHED'].includes(state.status)?<LiveChallengeAnalyticsPanel challengeId={state.id} stateVersion={state.stateVersion}/>:null}
