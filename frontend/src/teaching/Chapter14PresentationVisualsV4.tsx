...
import { Chapter14PresentationVisualV6, hasChapter14PresentationVisualV6 } from './Chapter14PresentationVisualsV6';
...
/**
 * Stable presentation facade. Chapter 14 currently uses V6 until the MASTER
 * renderer and its design system are committed atomically. This keeps main
 * buildable between commits.
 */
export function hasChapter14PresentationVisualV4(beat:LessonPresentationBeat){
  return hasChapter14PresentationVisualV6(beat)||hasChapter13PresentationVisual(beat)||hasChapter2ActivityVisual(beat)||hasChapter2InternetVisual(beat)||hasChapter2DeviceVisual(beat)||hasChapter2PresentationVisual(beat)||hasChapter1PresentationVisual(beat)||hasChapter14PresentationVisualV3(beat)||hasChapter14PresentationVisual(beat);
}
...
export function Chapter14PresentationVisualV4({beat,reveal}:{beat:LessonPresentationBeat;reveal:number}){
  ...
  if(hasChapter14PresentationVisualV6(beat))return <><Chapter14PresentationVisualV6 beat={beat} reveal={reveal}/><Chapter14PresentationCompleteness beat={beat}/>{beat.id==='h14p-142-practice'?<Chapter14PracticeQ4 reveal={reveal}/>:null}</>;
  if(hasChapter14PresentationVisualV3(beat))return <Chapter14PresentationVisualV3 beat={beat} reveal={reveal}/>;
  return <Chapter14PresentationVisual beat={beat} reveal={reveal}/>;
}
