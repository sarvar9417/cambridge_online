...
describe('Hodder source readiness gate', () => {
  ...
  it('derives the 9618 implementation queue from source readiness', () => {
    ...
    expect(sourceLocked9618HodderChapters()).toEqual([1, 2, 3, 7, 13, 14]);
    expect(unresolved9618HodderChapters()).toEqual([4, 5, 6, 8, 9, 10, 11, 12, 15, 16, 17, 18, 19, 20]);
  });
  ...
  it('keeps every 0478 chapter behind unresolved 9618 work', () => {
    ...
    expect(NEXT_HODDER_BUILD_TARGET).toEqual({ syllabus: '9618', chapter: 4 });
    expect(NEXT_HODDER_BUILD_TARGET_READINESS?.status).toBe('source-unresolved');
  });

  it('advances the compatibility queue to Chapter 4 after Chapter 3 is source-locked', () => {
    expect(NEXT_9618_HODDER_CHAPTER).toBe(4);
    expect(NEXT_9618_HODDER_CHAPTER_READINESS?.chapter).toBe(4);
