...
const chapter7BookWithCheckpoints = withChapter7PastPaperCheckpoints(CHAPTER_7_BOOK_SLIDES);

/**
 * Complete Chapter 7 presenter route.
 *
 * Invariant: the original guided-discovery lesson remains first and unchanged.
 * The coursebook deep-dive is appended only after that sequence, with live
 * Cambridge 0478 checkpoints inserted after the final slide of each 7.1–7.9 part.
 */
export const CHAPTER_7 = {
  ...DISCOVERY_CHAPTER_7,
  subtitle: 'Guided discovery first, then the complete Chapter 7 coursebook deep dive with live Cambridge 0478 past-paper checkpoints.',
  subtopics: [
  ...
  coverage: `15-slide discovery lesson + ${CHAPTER_7_BOOK_SLIDES.length}-slide complete book deep dive + ${CHAPTER_7_PAST_PAPER_CHECKPOINTS.length} live 0478 checkpoints`,
