...
const TOPIC_11:AuthoredSceneSpec[]=[
  ...
  s({id:'h1p-112-signed-extension',slideId:'h1-112-signed',role:'challenge',eyebrow:'1.1.2 · EXTEND',title:'Extend the signed pattern to 16 bits',block:{index:2},keyTerms:[2,3]}),
...
const TOPIC_12:AuthoredSceneSpec[]=[
  s({id:'h1p-121-bitmap-core',slideId:'h1-121-bitmap-basics',role:'concept',eyebrow:'1.2.1 · BITMAP MODEL',lead:true,keyTerms:[0,1]}),
  s({id:'h1p-121-bitmap-depth',slideId:'h1-121-bitmap-basics',role:'visual',eyebrow:'1.2.1 · PIXEL DATA',title:'Resolution and colour depth control bitmap data volume',block:{index:0},keyTerms:[2,3]}),
  s({id:'h1p-bitmap-resolution-compare',slideId:'h1-bitmap-resolution',role:'compare',eyebrow:'BITMAP QUALITY · COMPARE',lead:true,block:{index:0},keyTerms:[4,5]}),
...
export function chapter1PresentationStoryboard(topicCode:string,slides:readonly HodderLessonSlide[]):LessonPresentationBeat[]|null {
  if(topicCode==='overview')return [
    authoredStaticScene('h1p-overview-prior-activity','h1-prior','hook','CHAPTER 1 · PRIOR ACTIVITY','Reconstruct the source binary-addition task',[1],{bullets:['Carry out these binary additions and convert each answer to denary:']}),
    ...buildAuthoredStoryboard(slides,OVERVIEW),
  ];
  const opening=topicCode==='1.1'?[authoredStaticScene('h1p-11-hook','h1-111-number-systems','hook','1.1 · STARTER','How can the same bits represent numbers, text and machine data?',[2],{lead:'Representation only works when the system knows the base, width or character encoding used to interpret the stored pattern.'}),authoredStaticScene('h1p-11-objectives','h1-111-number-systems','objective','1.1 · LESSON GOALS','By the end of 1.1 you should be able to…',[2,15],{bullets:['Convert and calculate with binary, denary and hexadecimal.','Represent signed integers with two’s complement and explain overflow.','Use BCD, ASCII and Unicode appropriately.','Distinguish decimal and binary storage prefixes.']})]:topicCode==='1.2'?[authoredStaticScene('h1p-12-hook','h1-121-bitmap-basics','hook','1.2 · STARTER','A photograph and a sound clip are both bits. What changes between them?',[15,19],{lead:'The stored bits need a representation model: pixels for a bitmap, geometric objects for vectors, and sampled amplitude values for sound.'}),authoredStaticScene('h1p-12-objectives','h1-121-bitmap-basics','objective','1.2 · LESSON GOALS','By the end of 1.2 you should be able to…',[15,21],{bullets:['Explain bitmap, vector and sampled-sound representations.','Calculate raw bitmap storage from resolution and colour depth.','Explain the quality/file-size effects of image and sound sampling choices.','Choose an appropriate representation for a multimedia task.']})]:topicCode==='1.3'?[authoredStaticScene('h1p-13-hook','h1-13-need','hook','1.3 · STARTER','Can every file be made smaller without losing anything?',[21],{lead:'Compression decisions depend on whether exact reconstruction is required and whether repeated or perceptually unimportant data can be exploited.'}),authoredStaticScene('h1p-13-objectives','h1-13-need','objective','1.3 · LESSON GOALS','By the end of 1.3 you should be able to…',[21,24],{bullets:['Distinguish lossy and lossless compression.','Explain why MP3/JPEG-style reduction can be acceptable for media.','Trace run-length encoding for text and images.','Explain how sampling rate, resolution, frame rate and image properties affect file size.']})]:[];
  ...
  if(topicCode==='1.1')result.push(authoredStaticScene('h1p-11-recap','h1-115-unicode','recap','1.1 · RETRIEVAL','Rebuild the data-representation route from memory',[2,15],{bullets:['Why does binary fit two-state hardware?','How do you convert and calculate with binary and hexadecimal?','How does two’s complement represent a negative integer?','When would BCD, ASCII or Unicode be the right representation?']}));
  if(topicCode==='1.2')result.push(authoredStaticScene('h1p-12-recap','h1-124-video','recap','1.2 · RETRIEVAL','Choose the right representation and justify it',[15,21],{bullets:['Bitmap: explain resolution, colour depth and raw file size.','Vector: explain object/attribute storage and scaling.','Sound: explain sampling rate, sampling resolution and digitisation.','State one quality-versus-size trade-off for each medium.']}));
  if(topicCode==='1.3')result.push(authoredStaticScene('h1p-13-recap','h1-132-general','exam','1.3 · CAMBRIDGE CHECK','Compression: explain the method, not just the label',[21,26],{activity:{title:'30-second transfer',prompt:'Explain when lossless compression is required, then trace how RLE would encode one run of repeated values.',reveal:'Lossless is required when the exact original data must be reconstructed. RLE replaces an adjacent repeated run with its count and value; it is effective when long runs occur.'}}));
  return result;
}

export function chapter1PresentationClosing():LessonPresentationBeat[] { return [authoredStaticScene('h1p-chapter-activity','h1-activity-1i','challenge','CHAPTER 1 · ACTIVITY 1I','Use the chapter concepts together',[25],{bullets:['Define lossy and lossless compression and give examples.','Describe sound digitisation and why compressed music can remain acceptable.','Explain RLE with an example.','Compare bitmap and vector images and justify a representation for a realistic scenario.']}),authoredStaticScene('h1p-chapter-review','h1-hodder-review','recap','CHAPTER 1 · RETRIEVAL','What must you be able to reconstruct without notes?',[25,26],{bullets:['Signed binary: interpretation, creation, range and overflow.','BCD: representation, arithmetic and applications.','Bitmap/sound: sampling, resolution, file size and metadata.','Compression: choice, RLE and quality trade-offs.','Binary/hexadecimal: arithmetic, conversions and practical uses.']})]; }
