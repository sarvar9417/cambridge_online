...
const termsSlide = (
  ...
  section,
  subtopicCode,
  eyebrow: `${subtopicCode} · ZARUR ATAMALAR · ${index + 1}/${total}`,
  title: subtopicCode === '2.1' ? 'Networking terms you must use precisely' : 'Internet terms you must use precisely',
  lead: 'Kitobdagi qalin yozilgan atamalar: rasmiy ta’rif + qisqa, tushunarli izoh. Imtihonda aynan shu ta’riflar bilan ball olinadi.',
  keyTerms: group.map(item => ({
    term: item.term,
    definition: `${item.definition} Oddiy izoh: ${item.simple}`,
  })),
...
export const chapter2EmphasisBoardSlide = (): Chapter2EmphasisSlide => {
  ...
  return {
    ...
    subtopicCode: '2.2',
    eyebrow: 'CHAPTER 2 · QALIN YOZILGAN JOYLAR · TO‘LIQ RO‘YXAT',
    title: 'Kitobda qalin yozilgan barcha atama va iboralar',
    lead: 'Supplied PDFdan ajratib olingan har bir qalin (bold) atama, ibora, jadval va figura nomi — chop etilgan kitob sahifasi bo‘yicha guruhlangan.',
...
export const chapter2EmphasisBoardSlides = (): Chapter2EmphasisSlide[] => {
  ...
  return groups.map((bullets, index) => {
    ...
    return {
      ...full,
      id: `${full.id}-${index + 1}`,
      eyebrow: `CHAPTER 2 · QALIN YOZILGAN JOYLAR · ${index + 1}/${groups.length}`,
      title: `Kitobdagi muhim ajratilgan tushunchalar · ${index + 1}/${groups.length}`,
      bullets,
      sourcePages,
      sourceElements: [
