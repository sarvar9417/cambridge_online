...
const atomsBySlide = new Map<string, typeof CHAPTER_7_SOURCE_ATOMS>();
for (const atom of CHAPTER_7_SOURCE_ATOMS) {
 ...
 * terms, named examples, exact values/code fragments, activities, figures,
 * tables, review items and exam-style question identifiers are attached to the
 * presenter route rather than being represented only by a page-level summary.
 */
export const CHAPTER_7_SOURCE_ATOM_COMPLETE_SLIDES: LessonSlide[] =
  CHAPTER_7_FINAL_SOURCE_SLIDES.map(sourceBlock);

export const CHAPTER_7_SOURCE_ATOM_COVERAGE = {
  atoms: CHAPTER_7_SOURCE_ATOMS.length,
