...
const slides: HodderLessonSlide[] = [
  ...
  },
  {
    ...
    formula:'raw bits = width × height × bits per pixel',
    example:{title:'1920 × 1080 at 24-bit',lines:['1920 × 1080 × 24 = 49,766,400 bits','÷ 8 = 6,220,800 bytes'],answer:'Convert to the requested SI or IEC unit only after calculating bytes.'},
    richBlocks:[
      {kind:'bullets',items:['A bitmap header can store file type, file size, image dimensions/resolution, bit depth and compression information.','Cropping, reducing resolution or reducing colour depth reduces data volume but may reduce quality.']},
      {kind:'callout',tone:'extension',title:'Extension Activity 1C',text:'Calculate the storage needed for a UHD television screen image.'},
    ],visual:'pixels',accent:'amber',
  },
...

  {
    ...
    richBlocks:[
      ...
      {kind:'callout',tone:'info',title:'Figure 1.4',text:'The robot illustration demonstrates a picture built from a small collection of geometric objects rather than a dense pixel matrix.'},
  ...
  checkpoint('h1-cp-vector','1.2 Multimedia','Past papers: vector representation and storage',['1.2-lo-02'],[18],'emerald'),
  {
    ...
    richBlocks:[
      {kind:'table',table:{caption:'Vector vs bitmap',headers:['Decision factor','Vector','Bitmap'],rows:[['Structure','Geometric objects + attributes','Pixels'],['Scaling','No raster pixelation','Can become pixelated'],['Realism','Usually less photo-realistic','Well suited to photographs'],['Editing','Edit objects','Edit pixels'],['Typical files','.svg / .cgm / .odg','.jpeg / .bmp / .png']]}},
      {kind:'bullets',items:['Logo or scale drawing → vector is often the stronger choice.','Photograph editing → bitmap is usually the stronger choice.','Restrictions on destination file formats can override an otherwise ideal representation.']},
    ],visual:'vectors',accent:'emerald',
  },
...

  {
    ...
    richBlocks:[
      ...
      {kind:'callout',tone:'info',title:'Figures 1.5 and 1.6',text:'The first visual contrasts high- and low-frequency waves; the second overlays sampled amplitude values on a time axis to make digitisation concrete.'},
    ],visual:'sound',accent:'cyan',
  },
  checkpoint('h1-cp-sound-digitise','1.2 Multimedia','Past papers: analogue sound digitisation',['1.2-lo-04'],[19],'cyan'),
  {
    ...
    richBlocks:[
      {kind:'comparison',leftTitle:'Increase sampling quality',rightTitle:'Cost',rows:[['More accurate wave shape','Larger file'],['Larger dynamic range / less distortion','Longer transfer/download'],['Better recorded sound','More processing/storage required']]},
      {kind:'steps',title:'Digitising a clip',items:['Measure amplitude at fixed time intervals.','Approximate each amplitude to an available level.','Encode the sequence of values as binary.']},
    ],visual:'sound',accent:'cyan',
  },
  checkpoint('h1-cp-sampling','1.2 Multimedia','Past papers: sampling rate and sampling resolution',['1.2-lo-05'],[20],'cyan'),
  {
    ...
    richBlocks:[
      {kind:'bullets',items:['Trim start/stop times or duration.','Extract, save or delete part of a sample.','Alter frequency and amplitude.','Fade in / fade out.','Mix or merge tracks and combine multiple sources.','Reduce noise or isolate a wanted signal.','Convert between audio file formats.']},
    ],visual:'sound',accent:'emerald',
  },
  {
    ...
    richBlocks:[
      {kind:'bullets',items:['Digital cameras/phones can form moving images from sequences of still frames.','Light-sensitive sensors convert the captured image to electronic data.','The digital-video data is stored in a compressed format.','Frame rate is the number of frames recorded per second.','Motion JPEG is described as one frame-based approach in the coursebook.']},
    ],visual:'pixels',accent:'rose',
  },
...

  {
    ...
    richBlocks:[
      {kind:'comparison',leftTitle:'Lossless',rightTitle:'Lossy',rows:[['Preserves all original information','Discards selected information'],['Used when any loss is unacceptable','Used when perceptual loss is acceptable'],['Typically less aggressive reduction','Often achieves greater reduction']]}},
  ...
  checkpoint('h1-cp-compression-need','1.3 File compression','Past papers: why compression is needed',['1.3-lo-01'],[21],'rose'),
  {
    ...
    richBlocks:[
      ...
      {kind:'callout',tone:'extension',title:'Extension Activity 1D',text:'Research how photograph compression reduces file size without an obvious quality loss, then compare the principle with RLE.'},
    ],visual:'compression',accent:'rose',
  },
...

  {
    ...
    richBlocks:[
      {kind:'steps',title:'Basic RLE',items:['Find a run of identical adjacent items.','Store the run length.','Store the item/code being repeated.','Repeat for the next run.']},
      {kind:'code',title:'Text example',lines:['aaaaabbbbccddddd','05 97   04 98   02 99   05 100','16 original bytes → 8 encoded values/bytes in the simplified model']},
      {kind:'callout',tone:'warning',title:'When runs are short',text:'Alternating text can expand under naive RLE. The source introduces a flag byte so only worthwhile repeated sequences are encoded as runs; ordinary bytes can otherwise be stored directly.'},
  ...
  },
  {
    ...
    richBlocks:[
      {kind:'table',table:{caption:'Source comparison',headers:['Example','Uncompressed simplified size','RLE simplified size'],rows:[['8×8 black/white F','64 bytes','30 values/bytes'],['8×8 RGB image','192 bytes','92 values/bytes']]}},
      {kind:'paragraph',text:'Real compressed files also need metadata such as headers, so classroom percentage reductions are simplified demonstrations rather than complete file-format specifications.'},
    ],visual:'compression',accent:'emerald',
  },
  checkpoint('h1-cp-rle','1.3 File compression','Past papers: run-length encoding for text, images or sound',['1.3-lo-04'],[22,23,24],'emerald'),
  {
    ...
    richBlocks:[
      {kind:'table',table:{caption:'General reduction strategies',headers:['Media','Possible reductions'],rows:[['Movie/sound','Lower sampling rate; lower sampling resolution; lower frame rate'],['Image','Crop; lower colour/bit depth; lower image resolution']]}},
      {kind:'callout',tone:'warning',title:'Quality consequence',text:'These choices reduce the information captured or retained. The smaller file is obtained by changing source quality/quantity, not by magically storing the identical raw data in fewer bits.'},
    ],visual:'compression',accent:'amber',
  },
  {
    ...
    richBlocks:[
      {kind:'bullets',items:['Define lossy and lossless compression and give examples.','Describe microphone sound digitisation and why music is compressed while keeping acceptable quality.','Explain RLE with an example.','Compare bitmap and vector images and justify a representation for realistic software images.']},
  ...
  },
  {
    ...
    richBlocks:[
      {kind:'bullets',items:['Two’s-complement interpretation, creation, range and overflow.','BCD representation, arithmetic and applications.','Sampling resolution, bitmap resolution/file size, header metadata and sound-editing features.','Sampling + compression choice + RLE on an image.','Binary arithmetic, hexadecimal definition/use/conversion and base conversions.']},
      {kind:'callout',tone:'info',title:'Why this is not the live checkpoint',text:'The Hodder review includes older exam-style material. CamPath keeps it as coursebook coverage, while lesson checkpoints query only approved 2021–2025 corpus questions mapped to the part just taught.'},
    ],visual:'recap',accent:'indigo',
  },
];
...
export const HODDER_CHAPTER_1: HodderLessonChapter = {
  number:1,
  level:'AS Level',
  title:'Information representation and multimedia',
