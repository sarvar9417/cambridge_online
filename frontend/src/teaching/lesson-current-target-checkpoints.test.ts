...
describe('9618 Lesson Studio current-target checkpoint contract',()=>{
  it('maps every live Chapter 1 and Chapter 13 checkpoint to a current 2026–2028 LO',()=>{
  ...
  it('keeps split/combined current syllabus objectives explicit instead of guessing from one historical alias',()=>{
    ...
    expect(byId.get('h1-cp-lossy-lossless')?.learningObjectiveCodes).toEqual(['1.3.2']);
    expect(byId.get('h1-cp-compression-choice')?.learningObjectiveCodes).toEqual(['1.3.2']);
    expect(byId.get('h13-cp-file-org')?.learningObjectiveCodes).toEqual(['13.2.1']);
    expect(byId.get('h13-cp-org-access-choice')?.learningObjectiveCodes).toEqual(['13.2.1']);
