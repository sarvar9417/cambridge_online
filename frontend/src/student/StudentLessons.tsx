...
function Visual({ kind }: { kind?: LessonVisual }) {
  ...
  return <div className={`lesson-visual lesson-visual-${kind}`} aria-hidden="true">
    {labels[kind].map((item, index) => <span key={`${item}-${index}`}>{item}</span>)}
  </div>;
}

function WaveFigure({ figure }: { figure: Extract<LessonFigure, { kind:'wave' }> }) {
  const width = 620;
  const height = figure.series.length * 118;
  return <div className="hodder-figure hodder-wave-figure">
    <strong>{figure.title}</strong>
    <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label={figure.title}>
...
function FigureBlock({ figure }: { figure: LessonFigure }) {
  ...
  if (figure.kind === 'grid') return <figure className="hodder-figure hodder-grid-figure">
    <strong>{figure.title}</strong>
    <div className="hodder-pixel-grid" style={{ gridTemplateColumns:`repeat(${Math.max(...figure.rows.map((row) => row.length))}, 1fr)` }}>
      {figure.rows.flatMap((row, rowIndex) => [...row].map((symbol, columnIndex) => <span data-symbol={symbol} key={`${rowIndex}-${columnIndex}`}>{symbol}</span>))}
    </div>
    {figure.legend && <div className="hodder-figure-legend">{figure.legend.map((item) => <span key={`${item.symbol}-${item.label}`}><b data-symbol={item.symbol}>{item.symbol}</b>{item.label}</span>)}</div>}
    {figure.caption && <figcaption>{figure.caption}</figcaption>}
  </figure>;
  if (figure.kind === 'sequence') return <figure className="hodder-figure hodder-sequence-figure">
    <strong>{figure.title}</strong>
    <div className="hodder-sequence">{figure.items.map((item, index) => <div className="hodder-sequence-node-wrap" key={`${item.label}-${index}`}><div className="hodder-sequence-node"><b>{item.label}</b>{item.note && <small>{item.note}</small>}</div>{index < figure.items.length - 1 && <span className="hodder-sequence-arrow">→</span>}</div>)}</div>
    {figure.caption && <figcaption>{figure.caption}</figcaption>}
  </figure>;
  if (figure.kind === 'bitfield') return <figure className="hodder-figure hodder-bitfield-figure">
    <strong>{figure.title}</strong>
    <div className="hodder-bitfield-list">{figure.fields.map((field, index) => <div className="hodder-bitfield-row" key={`${field.label}-${index}`}><div><b>{field.label}</b>{field.detail && <small>{field.detail}</small>}</div><div className="hodder-bitfield-bits">{field.bits.split(/\s+/).filter(Boolean).map((bit, bitIndex) => bit === '|' ? <i key={bitIndex} /> : <span key={bitIndex}>{bit}</span>)}</div></div>)}</div>
    {figure.caption && <figcaption>{figure.caption}</figcaption>}
  </figure>;
  return <figure className="hodder-figure hodder-pixel-scale-figure">
    <strong>{figure.title}</strong>
    <div className="hodder-pixel-scale">{figure.stages.map((stage, index) => <div className={`hodder-pixel-stage level-${Math.max(1, Math.min(5, stage.level))}`} key={`${stage.label}-${index}`}><div aria-hidden="true">{Array.from({ length:16 }, (_, i) => <span key={i} />)}</div><b>{stage.label}</b>{stage.note && <small>{stage.note}</small>}</div>)}</div>
    {figure.caption && <figcaption>{figure.caption}</figcaption>}
  </figure>;
}
...
function RichBlock({ block }: { block: LessonRichBlock }) {
  ...
  if (block.kind === 'source-note') return <aside className="hodder-source-note student-source-note"><header><span>MANBA IZOHİ</span><strong>{block.title}</strong></header><div><section><b>{block.sourceLabel}</b><p>{block.sourceText}</p></section><section><b>{block.examSafeLabel}</b><p>{block.examSafeText}</p></section></div></aside>;
...
function StudentCheckpoint({ slide }: { slide: StudySlide }) {
  ...
  return <section className="student-checkpoint">
    <span>CAMBRIDGE CHECKPOINT</span>
    <h2>{slide.title}</h2>
    <p>{slide.checkpointUnavailableReason ?? slide.lead}</p>
    {codes.length > 0 && <div>{codes.map((code) => <code key={code}>{code}</code>)}</div>}
    <p className="student-checkpoint-safety">Student oynasi diagramma yoki dependency kerak bo‘ladigan past-paper savolini matnning bir qismi bilan ko‘rsatmaydi. Tayyor, to‘liq mashqlar O‘rganish bo‘limida ochiladi.</p>
...
function GenericStudySlide({ slide }: { slide: StudySlide }) {
  ...
  return <div className="student-study-slide-body">
    <div className="lesson-copy hodder-copy">
      ...
      {slide.teacherPrompt && <aside className="lesson-prompt student-self-check"><span>O‘ZINGNI TEKSHIR</span><p>{slide.teacherPrompt}</p></aside>}
      {slide.activity && <details className="lesson-activity"><summary><span>MASHQ</span><strong>{slide.activity.title}</strong></summary><p>{slide.activity.prompt}</p>{slide.activity.reveal && <div className="lesson-activity-answer"><span>JAVOB / YO‘L-YO‘RIQ</span><p>{slide.activity.reveal}</p></div>}</details>}
      {slide.sourcePages?.length ? <p className="student-source-pages">Manba sahifalari: {slide.sourcePages.join(', ')}</p> : null}
    </div>
...
export function StudentLessons() {
  ...
  if (!chosen) {
    return <section className="student-lessons-library">
      <header>
        <div>
          <p className="lesson-eyebrow">STUDY MODE</p>
          <h1>Darslar</h1>
          <p>Teacher Studio bilan bir xil source-backed lesson ma’lumotlari. Bu ko‘rinish o‘quvchi mustaqil o‘qishi, misollarni ko‘rishi va mashqlarni ochib tekshirishi uchun soddalashtirilgan.</p>
        </div>
        <span>{STUDENT_STUDY_CHAPTERS.length} chapter</span>
      </header>
      <div className="student-lessons-grid">{STUDENT_STUDY_CHAPTERS.map((chapter) => <button type="button" key={chapter.number} onClick={() => navigate(`oquvchi/darslar?chapter=${chapter.number}&slide=${chapter.slides[0]?.id ?? ''}`)}>
        <span className="student-chapter-number">{String(chapter.number).padStart(2, '0')}</span>
        <small>{chapter.level}</small>
        <h2>{chapter.title}</h2>
        <p>{chapter.subtitle}</p>
        <div>{chapter.subtopics.map((item) => <span key={item}>{item}</span>)}</div>
        <footer><strong>{chapter.slides.length} qism</strong><span>O‘rganishni boshlash →</span></footer>
      </button>)}</div>
    </section>;
  }
  ...
  const sections = useMemo(() => [...new Set(chosen.slides.map((item) => item.section))], [chosen]);
  const go = (index: number) => {
    const target = chosen.slides[Math.max(0, Math.min(chosen.slides.length - 1, index))];
    if (target) navigate(`oquvchi/darslar?chapter=${chosen.number}&slide=${target.id}`);
  };
  ...
  return <section className={`student-study-mode accent-${slide.accent || 'indigo'}`}>
    <header className="student-study-header">
      <button type="button" className="secondary" onClick={() => navigate('oquvchi/darslar')}>← Chapters</button>
      <div><small>{chosen.level} · Chapter {chosen.number}</small><strong>{chosen.title}</strong></div>
      <span>{slideIndex + 1}/{chosen.slides.length}</span>
    </header>
    ...
    <div className="student-study-layout">
      <aside className="student-study-outline" aria-label="Dars bo‘limlari">
        <p>BO‘LIMLAR</p>
        {sections.map((section, index) => <button type="button" className={slide.section === section ? 'active' : ''} key={section} onClick={() => go(sectionStarts[index] ?? 0)}><span>{String(index + 1).padStart(2, '0')}</span>{section}</button>)}
      </aside>
      <main className="student-study-paper">
        {chosen.number === 7 && !slide.examPractice ? <Chapter7SlideBody slide={slide} /> : <GenericStudySlide slide={slide} />}
      </main>
    </div>
    <footer className="student-study-nav">
      <button type="button" className="secondary" disabled={slideIndex === 0} onClick={() => go(slideIndex - 1)}>← Oldingi</button>
      <div>{chosen.slides.map((item, index) => <button type="button" key={item.id} aria-label={`${index + 1}-qism`} className={index === slideIndex ? 'active' : item.section === slide.section ? 'same-section' : ''} onClick={() => go(index)} />)}</div>
      <button type="button" disabled={slideIndex === chosen.slides.length - 1} onClick={() => go(slideIndex + 1)}>Keyingi →</button>
    </footer>
  </section>;
}
