...
export function AnalyticsPanel({ classes, owner }: { classes: ClassItem[]; owner: boolean }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .catch((cause) => setError(cause instanceof Error ? cause.message : 'Analitika yuklanmadi.'));
  }, [classId, owner]);
  ...
  return (
    <section className="analytics-panel">
      ...
      {students.length > 0 && (
        <div
          className="heatmap"
          style={{ gridTemplateColumns: `minmax(140px,1fr) repeat(${topics.length},52px)` }}
        >
          <b>O‘quvchi</b>
          {topics.map((topic) => (
            <b key={topic}>T{topic}</b>
          ))}
          {students.flatMap(([id, name]) => [
            <strong key={`${id}-name`}>{name}</strong>,
            ...topics.map((topic) => {
              const cell = heat.find((item) => item.studentId === id && item.topic === topic);
              const pct =
                cell?.mastery === null || cell?.mastery === undefined
                  ? null
                  : Math.round(cell.mastery * 100);
              return (
                <span
                  key={`${id}-${topic}`}
                  className={`heat-cell ${pct === null || Number(cell?.evidence) < 15 ? 'low-data' : pct < 50 ? 'weak' : pct <= 75 ? 'mid' : 'strong'}`}
                  title={`${name}, Topic ${topic}: ${pct ?? 'ma’lumot yo‘q'}${pct === null ? '' : `%, ${cell?.evidence} ball dalil`}`}
                >
                  {pct === null ? '—' : pct}
                </span>
              );
      ...
      <div className="analytics-columns">
        <div>
          <h3>Yo‘qotilgan mark pointlar</h3>
          {points.length === 0 ? (
            <p className="empty">Kamida 8 ta tekshirilgan javob kerak.</p>
          ) : (
            points.map((item) => (
              <div className="metric-row" key={item.id}>
                <span>
                  <strong>
                    {item.code} · {item.displayRef}
                  </strong>
        ...
        <div>
          <h3>Command word</h3>
          {words.length === 0 ? (
            <p className="empty">Kamida 10 ta baholangan javob kerak.</p>
          ) : (
            words.map((item) => (
      ...
      {owner && quality.length > 0 && (
        <div>
          <h3>AI sifati</h3>
          {quality.map((item) => (
            <div className="quality-row" key={item.promptVersion}>
              <strong>{item.promptVersion}</strong>
              <span>Moslik {item.pointAgreement}%</span>
              <span>FP {item.falsePositive}%</span>
              <span>FN {item.falseNegative}%</span>
