...
describe('computeScore', () => {
  it('all_required: 3 dan 2 tasi → 2', () =>
    expect(computeScore(base(), m('MP1', 'MP2')).score).toBe(2));
  it('any_3_from_5: 4 tasi mos → 3', () => {
    const s = base('any_n_from_m');
    s.points = ['MP1', 'MP2', 'MP3', 'MP4', 'MP5'].map((code) => ({
      code,
      marks: 1,
      groupId: 'g',
      requires: [],
  ...
  it('requires dependency cancels point', () => {
    const s = base();
    s.points = [{ code: 'MP2', marks: 1, groupId: null, requires: ['MP1'] }];
    expect(computeScore(s, m('MP2')).score).toBe(0);
  });
  it('chained requires stabilizes', () => {
    const s = base();
    s.points[1]!.requires = ['MP1'];
    s.points[2]!.requires = ['MP2'];
  ...
  it('two groups have separate caps', () => {
    ...
    s.maxMarks = 2;
    s.points = [
      { code: 'A', marks: 1, groupId: 'a', requires: [] },
      { code: 'B', marks: 1, groupId: 'a', requires: [] },
      { code: 'C', marks: 1, groupId: 'b', requires: [] },
    ];
    s.groups = [
      { id: 'a', nRequired: 1, marksPerPoint: 1, maxMarks: 1 },
      { id: 'b', nRequired: 1, marksPerPoint: 1, maxMarks: 1 },
    ];
    expect(computeScore(s, m('A', 'B', 'C')).score).toBe(2);
  });
  it('group and ungrouped points combine', () => {
    const s = base('any_n_from_m');
    s.points[0]!.groupId = 'g';
    s.points[1]!.groupId = 'g';
    s.groups = [{ id: 'g', nRequired: 1, marksPerPoint: 1, maxMarks: 1 }];
    expect(computeScore(s, m('MP1', 'MP2', 'MP3')).score).toBe(2);
  });
  it('exact match false is zero', () =>
    expect(computeScore(base('exact_match'), m('MP1')).score).toBe(0));
  it('levels needs teacher', () =>
    expect(computeScore(base('levels_of_response'), m()).score).toBeNull());
  it('never exceeds max marks', () => {
    const s = base();
    s.maxMarks = 1;
    expect(computeScore(s, m('MP1', 'MP2', 'MP3')).score).toBe(1);
  });
  it('empty input is zero', () => expect(computeScore(base(), []).score).toBe(0));
  it('unknown code creates finding', () =>
    expect(computeScore(base(), m('MP9')).findings).toContain('unknown_mark_point:MP9'));
});
