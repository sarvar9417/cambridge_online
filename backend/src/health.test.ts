...
describe('readiness', () => {
  it('returns ok when PostgreSQL responds', async () => {
    ...
    expect(response.body).toEqual({ status: 'ok', database: 'ok', capabilities: {} });
  });
  it('returns 503 when PostgreSQL is unavailable', async () => {
    const app = express();
    app.use(
      '/ready',
      createReadyRouter({ query: vi.fn().mockRejectedValue(new Error('down')) } as unknown as Pool),
    );
    const response = await request(app).get('/ready');
  ...
  it('returns 503 when database configuration is missing', async () => {
    const app = express();
    app.use('/ready', createReadyRouter(null));
