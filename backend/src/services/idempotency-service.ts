...
export function requestFingerprint(method: string, path: string, body: unknown) {
  return createHash('sha256').update(JSON.stringify({ method, path, body:body ?? null })).digest('hex');
}
...
export class IdempotencyService {
  ...
  async claim(actorId: string, key: string, requestHash: string): Promise<IdempotencyClaim> {
    ...
    if (!existing.rowCount) {
      await this.pool.query(`delete from idempotency_records where actor_id=$1 and key=$2 and expires_at<=now()`, [actorId, key]);
