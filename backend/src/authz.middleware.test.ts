...
const actorMiddleware: express.RequestHandler = (req, _res, next) => {
  req.actor = actor;
  next();
};
...
describe('authorization middleware', () => {
  ...
  it('requireClassAccess allows visible class', async () => {
    ...
    app.get('/classes/:classId', requireClassAccess({ query } as unknown as Pool), (_q, r) =>
      r.sendStatus(204),
    );
  ...
  it('class access query includes teacher, owner and student scopes', async () => {
    const query = vi.fn().mockResolvedValue({ rowCount: 1 });
    const app = express();
    app.use(express.json(), actorMiddleware);
    app.post('/x', requireClassAccess({ query } as unknown as Pool, 'body'), (_q, r) =>
      r.sendStatus(204),
    );
    await request(app).post('/x').send({ classId: 'class-id' });
    const [sql, values] = query.mock.calls[0]!;
    expect(sql).toContain("$2='student'");
    expect(sql).toContain('class_teachers');
    expect(values).toEqual(['class-id', 'teacher', 'school-id', 'user-id']);
  });
});
