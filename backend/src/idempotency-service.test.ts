...
describe('request idempotency', () => {
  ...
  it('claims a new key atomically', async () => {
    ...
    await expect(new IdempotencyService({ query } as unknown as Pool).claim('u', 'request-1', 'hash'))
  ...
  it('replays the saved status and body without claiming again', async () => {
    const query = vi.fn()
      ...
      .mockResolvedValueOnce({ rowCount:1,rows:[{ request_hash:'hash',status:'completed',response_status:202,response_body:{ id:'export' } }] });
    await expect(new IdempotencyService({ query } as unknown as Pool).claim('u', 'request-1', 'hash'))
  ...
  it('rejects reuse with a different request', async () => {
    const query = vi.fn()
      .mockResolvedValueOnce({ rowCount:0,rows:[] })
      .mockResolvedValueOnce({ rowCount:1,rows:[{ request_hash:'other',status:'completed' }] });
    await expect(new IdempotencyService({ query } as unknown as Pool).claim('u', 'request-1', 'hash'))
  ...
  it('rejects a concurrent duplicate while the first request is processing', async () => {
    const query = vi.fn()
      .mockResolvedValueOnce({ rowCount:0,rows:[] })
      .mockResolvedValueOnce({ rowCount:1,rows:[{ request_hash:'hash',status:'processing' }] });
    await expect(new IdempotencyService({ query } as unknown as Pool).claim('u', 'request-1', 'hash'))
