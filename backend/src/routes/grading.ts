...
export function createGradingRouter(service: GradingService) {
  ...
  router.get('/appeals', async (req, res) =>
    res.json({ data: await service.appealQueue(req.actor!) }),
  );
  ...
  router.post('/:id/confirm', async (req, res) =>
    res.json(await service.confirm(req.actor!, z.string().uuid().parse(req.params.id))),
  );
  router.post('/:id/appeal', async (req, res) => {
    const body = z.object({ reason: z.string().trim().min(10).max(1000) }).parse(req.body);
    res
      .status(201)
      .json(await service.appeal(req.actor!, z.string().uuid().parse(req.params.id), body.reason));
  });
