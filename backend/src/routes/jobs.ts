...
export function createJobsRouter(pool: Pool) {
  const router=Router();
  router.post('/run-once',async(req,res)=>{if(req.actor!.role==='student'){res.status(403).json({error:{code:'staff_only',message:'Faqat xodimlar uchun.'}});return}const runner=new JobRunner(pool,{'export-pdf':createExportPdfProcessor(pool)});res.json({processed:await runner.runOnce(['export-pdf'])})});
