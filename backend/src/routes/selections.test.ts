...
describe('selection management routes',()=>{
 const itemA='33333333-3333-4333-8333-333333333333',itemB='44444444-4444-4444-8444-444444444444';
 function managedApp(repository:Partial<PgSelectionsRepository>){const app=express();app.use(express.json());app.use((req,_res,next)=>{req.actor=teacher;next()});app.use('/selections',createSelectionsRouter(repository as PgSelectionsRepository));app.use((error:unknown,_req:express.Request,res:express.Response,_next:express.NextFunction)=>{if(error instanceof ZodError){res.status(400).json({error:{code:'validation_error'}});return}res.status(500).json({error:{code:'internal_error'}})});return app}
 it('renames an owned selection',async()=>{const rename=vi.fn().mockResolvedValue({id:selectionId,name:'Networks mock'});const response=await request(managedApp({rename})).patch(`/selections/${selectionId}`).send({name:'  Networks mock  '}).expect(200);expect(rename).toHaveBeenCalledWith(teacher,selectionId,'Networks mock');expect(response.body.name).toBe('Networks mock')});
