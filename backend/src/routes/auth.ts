...
import { loginSchema, redeemInviteSchema, registerSchema } from '../lib/auth-schemas.js';
      ...
      error.code === 'refresh_reused'
        ...
        : error.code === 'invalid_credentials'
          ...
          : error.code === 'invite_invalid'
            ? 'Taklif kodi yaroqsiz yoki ishlatilgan.'
            : error.code === 'username_taken'
              ? 'Bu username band.'
              : error.code === 'email_taken'
                ? 'Bu email allaqachon ro‘yxatdan o‘tgan.'
                : error.code === 'account_suspended'
                  ? 'Hisobingiz to‘xtatilgan. O‘qituvchingizga murojaat qiling.'
...
export function createAuthRouter(auth: AuthService) {
  ...
  router.post(
    '/register',
    rateLimit({ windowMs: 60 * 60_000, max: 5, key: (req) => `register:${req.ip}` }),
  ...
  router.post(
    ...
    async (req, res) => {
      try {
        const session = await auth.redeemInvite(req.body);
        setRefreshCookie(res, session);
        res.status(201).json({ accessToken: session.accessToken, user: session.user });
      } catch (error) {
        sendAuthError(res, error);
  ...
  router.post('/change-password', requireAuth(auth), async (req, res) => {
    try {
      const body = z
        ...
        .parse(req.body);
      await auth.changePassword(req.actor!.id, body.currentPassword, body.newPassword);
      res.clearCookie(COOKIE, { path: '/api/v1/auth' });
      res.status(204).end();
    } catch (error) {
      sendAuthError(res, error);
