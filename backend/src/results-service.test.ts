...
describe('results detail authorization and remediation', () => {
  ...
  it('maps a released visible legacy result with numeric scores and no invented practice target', async () => {
    ...
    const query = vi.fn().mockResolvedValue({
      ...
      rows: [{
        ...
        content_json:null,content_version:null,practice_targets:[],
      }],
    });
    ...
    await expect(new ResultsService({ query } as unknown as Pool).detail(student, 'submission-a')).resolves.toEqual([{
      ...
      contentJson:null,contentVersion:null,assetUrls:{},practiceTargets:[],
    }]);
    expect(query.mock.calls[0]![0]).toContain("compat.relation in('equivalent','subtopic_compatible')");
    expect(query.mock.calls[0]![0]).toContain('target_topic.syllabus_id=c.syllabus_id');
  });

  it('returns reviewed current-syllabus practice targets with a released result', async () => {
    const practiceTargets=[{subtopicId:'target-subtopic',code:'3.2',title:'Logic gates'}];
    const query=vi.fn().mockResolvedValue({rowCount:1,rows:[{
      id:'question-1',grading_id:'grading-1',appeal_status:null,display_ref:'3(a)',stem_md:'Explain',marks:4,
      text:'answer',final_score:'2',teacher_feedback_md:null,points:[],content_json:null,content_version:null,
      practice_targets:practiceTargets,
    }]});
    const [detail]=await new ResultsService({query} as unknown as Pool).detail(student,'submission-a');
    expect(detail?.practiceTargets).toEqual(practiceTargets);
  });
  ...
  it('returns source-backed result content with only short-lived signed assets', async () => {
    ...
    const query=vi.fn()
      .mockResolvedValueOnce({rowCount:1,rows:[{
        ...
        content_json:content,content_version:1,practice_targets:[],
      }]})
    ...
    expect(detail?.practiceTargets).toEqual([]);
