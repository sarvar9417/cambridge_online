...
import { createLiveExamBuilderCommandsRouter } from './routes/live-exam-builder-commands.js';
...
import { LiveExamBuilderCommandService } from './services/live-exam-builder-command-service.js';
...
export function createApp(
  ...
  // Builder reads are staff-only and must mount before the generic '/:id'
  // session snapshot so words like '/builder-options' are never parsed as UUIDs.
  if (pool) mountPrivate(
    '/api/v1/live-exams',
    createLiveExamBuilderRouter(new LiveExamBuilderService(pool)),
  );
  // Draft/publish builder commands reuse the canonical live_exam_* tables and
  // the same immutable Cambridge question/Mark Scheme snapshot contracts.
  if (pool && questionsRepository) mountPrivate(
    '/api/v1/live-exams',
    createLiveExamBuilderCommandsRouter(new LiveExamBuilderCommandService(pool, questionsRepository)),
  );
