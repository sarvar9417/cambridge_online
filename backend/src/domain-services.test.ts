...
describe('domain authorization', () => {
  ...
  it('starts a precreated not-started submission with a server timestamp',async()=>{
    ...
    await expect(new AssignmentsService(pool).start(student,'assignment-id','session-id'))
      .resolves.toMatchObject({submissionId:'submission-id',activeSessionId:'session-id',startedAt});
    expect(query.mock.calls[2]![0]).toContain("status=case when submissions.status='not_started' then 'in_progress'");
    expect(query.mock.calls[2]![0]).toContain("started_at=case when submissions.status='not_started' then now()");
  ...
  it('student appeal query requires ownership and release',async()=>{const query=vi.fn().mockResolvedValueOnce({}).mockResolvedValueOnce({rowCount:0,rows:[]}).mockResolvedValueOnce({}),release=vi.fn();await expect(new GradingService({connect:vi.fn().mockResolvedValue({query,release})}as unknown as Pool).appeal(student,'g','A sufficiently long reason')).rejects.toMatchObject({status:404});expect(query.mock.calls[1]![0]).toContain('g.released_at is not null');expect(query.mock.calls[2]![0]).toBe('rollback');expect(release).toHaveBeenCalled()});
  it('limits a student to three appeals in one assignment',async()=>{const query=vi.fn().mockResolvedValueOnce({}).mockResolvedValueOnce({rowCount:1,rows:[{submission_id:'submission'}]}).mockResolvedValueOnce({rowCount:0,rows:[]}).mockResolvedValueOnce({rows:[{count:3}]}).mockResolvedValueOnce({}),release=vi.fn();await expect(new GradingService({connect:vi.fn().mockResolvedValue({query,release})}as unknown as Pool).appeal(student,'g','A sufficiently long reason')).rejects.toMatchObject({code:'appeal_limit',status:409});expect(query.mock.calls[1]![0]).toContain('for update of s');expect(query.mock.calls[3]![1]).toEqual(['submission']);expect(query.mock.calls[4]![0]).toBe('rollback')});
  it('creates an appeal and returns the grading to teacher review',async()=>{const query=vi.fn().mockResolvedValueOnce({}).mockResolvedValueOnce({rowCount:1,rows:[{submission_id:'submission'}]}).mockResolvedValueOnce({rowCount:0,rows:[]}).mockResolvedValueOnce({rows:[{count:2}]}).mockResolvedValueOnce({rows:[{id:'appeal',status:'open'}]}).mockResolvedValueOnce({}).mockResolvedValueOnce({}),release=vi.fn();await expect(new GradingService({connect:vi.fn().mockResolvedValue({query,release})}as unknown as Pool).appeal(student,'g','A sufficiently long reason')).resolves.toEqual({id:'appeal',status:'open'});expect(query.mock.calls[5]![0]).toContain("status='needs_teacher'");expect(query.mock.calls[6]![0]).toBe('commit')});
