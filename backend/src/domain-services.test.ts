...
describe('domain authorization', () => {
  it('staff grading queue rejects students before querying the database', async () => {
    const query = vi.fn();
    await expect(
      new GradingService({ query } as unknown as Pool).queue(student),
  ...
  it('assignment attempt rejects staff before opening a transaction', async () => {
    const connect = vi.fn();
    await expect(
      new AssignmentsService({ connect } as unknown as Pool).start(owner, 'assignment-id'),
    ).rejects.toMatchObject({ status: 403 });
  ...
  it('assignment creation rejects students before opening a transaction', async () => {
    const connect = vi.fn();
    await expect(
      new AssignmentsService({ connect } as unknown as Pool).create(student, {
        classId: 'class-id',
        title: 'Test',
  ...
  it('answer save hides submissions owned by another student', async () => {
    ...
    await expect(
      new AssignmentsService({ query } as unknown as Pool).saveAnswer(
        student,
        'submission-id',
        'question-id',
        'answer',
  ...
  it('result detail always includes released-only and school/class scope', async () => {
    ...
    // Nothing visible is a 404, never an empty list; this case only asserts the
    // scope that the SQL carries.
    await expect(
      new ResultsService({ query } as unknown as Pool).detail(owner, 'submission-id'),
  ...
  it('heartbeat rejects a replaced device session', async () => {
    ...
    await expect(
      new AssignmentsService({ query } as unknown as Pool).heartbeat(
        student,
        'submission',
        'old-session',
      ),
    ).rejects.toMatchObject({ code: 'session_replaced', status: 409 });
  });
  ...
  it('heartbeat returns server-authoritative remaining time', async () => {
    ...
    const state = await new AssignmentsService({ query } as unknown as Pool).heartbeat(
      student,
      'submission',
      'session',
    );
    expect(state.remainingSeconds).toBeGreaterThan(1700);
    expect(state.status).toBe('in_progress');
  });
  it('staff cannot submit a student grading appeal', async () => {
    const query = vi.fn();
    await expect(
      new GradingService({ query } as unknown as Pool).appeal(
        owner,
        'g',
  ...
  it('student appeal query requires ownership and release', async () => {
    const query = vi.fn().mockResolvedValue({ rowCount: 0, rows: [] });
    await expect(
      new GradingService({ query } as unknown as Pool).appeal(
        student,
  ...
  it('students cannot read the staff appeal queue', async () => {
    const query = vi.fn();
    await expect(
      new GradingService({ query } as unknown as Pool).appealQueue(student),
    ).rejects.toMatchObject({ status: 403 });
    expect(query).not.toHaveBeenCalled();
  });
  it('students cannot resolve grading appeals', async () => {
    const connect = vi.fn();
    await expect(
      new GradingService({ connect } as unknown as Pool).resolveAppeal(
        student,
        'appeal',
        'accepted',
        'Recheck',
  ...
  it('students cannot read assignment-wide results', async () => {
    const query = vi.fn();
    await expect(
      new AssignmentsService({ query } as unknown as Pool).results(student, 'assignment'),
    ).rejects.toMatchObject({ status: 403 });
    expect(query).not.toHaveBeenCalled();
  });
  it('assignment results retain class authorization scope', async () => {
    ...
    const data = await new AssignmentsService({ query } as unknown as Pool).results(
      owner,
      'assignment',
    );
    expect(query.mock.calls[0]![0]).toContain("$2='owner' and c.school_id=$3");
