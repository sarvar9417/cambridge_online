...
describe('idempotent Express operation', () => {
  ...
  it('replays a completed response without invoking the operation', async () => {
    const query = vi.fn()
      ...
      .mockResolvedValueOnce({ rowCount:1,rows:[{
        ...
        status:'completed',response_status:201,response_body:{ id:'existing' },
      }] });
  ...
  it('releases a claim when the operation fails', async () => {
    ...
    await expect(runIdempotent(request(), response(), { query } as unknown as Pool, async () => { throw failure; }))
