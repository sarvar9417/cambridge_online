...
export interface ExportSchemeAsset{id:string;kind:string;contentMd:string;altText:string;sourcePage:number;sourceSha256?:string|null}
...
export interface ExportQuestion{displayRef:string;sourceRef?:string;stem:string;contentJson?:StructuredQuestionContent|null;context?:string;contextBlocks?:ExportContextBlock[];marks:number;answerLines?:number|null;role?:'graded'|'context_only';schemeStatus?:string;schemeGuidance?:string|null;schemeAssets?:ExportSchemeAsset[];points?:ExportSchemePoint[]}
...
function renderSchemeAssets(q:ExportQuestion){
  return (q.schemeAssets??[]).map(asset=>{
    if(isSvg(asset.contentMd)){
      return `<figure class="scheme-source-visual" data-source-page="${asset.sourcePage}" data-scheme-asset-id="${esc(asset.id)}"><img src="${svgDataUri(asset.contentMd)}" alt="${esc(asset.altText)}"></figure>`;
    }
    const table=markdownTable(asset.contentMd);
    return `<div class="scheme-source-visual" data-source-page="${asset.sourcePage}" data-scheme-asset-id="${esc(asset.id)}">${table??`<pre>${esc(asset.contentMd)}</pre>`}</div>`;
  }).join('');
}
function renderScheme(q:ExportQuestion){
  ...
  const visuals=renderSchemeAssets(q);
  return `${warning}${guidance}${visuals}<ol class="scheme">${points||'<li class="scheme-empty">No atomic mark-scheme points are available for this item.</li>'}</ol>`;
}
