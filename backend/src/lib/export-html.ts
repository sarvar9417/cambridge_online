...
const renderScheme=(q:ExportQuestion)=>`${q.schemeStatus&&q.schemeStatus!=='approved'?`<div class="scheme-warning">Mark scheme review status: ${esc(q.schemeStatus)} — source points are shown without promoting this review state.</div>`:''}<ol class="scheme">${(q.points??[]).map(p=>`<li><strong>${esc(p.code)}</strong> ${esc(p.text)} <span>[${p.marks}]</span></li>`).join('')}</ol>`;
