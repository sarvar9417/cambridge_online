...
describe('privacy service', () => {
  ...
  it('rejects anonymization by a student before querying the database', async () => {
    const connect = vi.fn();
    await expect(
      new PrivacyService({ connect } as unknown as Pool).anonymizeStudent(student, 'target-id'),
  ...
  it('hides a student outside the owner school and rolls back', async () => {
    ...
    const connect = vi.fn().mockResolvedValue({ query, release });
    await expect(
      new PrivacyService({ connect } as unknown as Pool).anonymizeStudent(owner, 'target-id'),
