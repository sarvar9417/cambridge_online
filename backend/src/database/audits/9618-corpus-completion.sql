...
begin
  ...
  join source_papers sp on sp.id=q.source_paper_id
  join syllabi sy on sy.id=sp.syllabus_id and sy.code='9618'
  join learning_objectives lo on lo.id=qlo.lo_id join subtopics st on st.id=lo.subtopic_id join topics t on t.id=st.topic_id
  where q.marks>0 and sp.source_url is not null and t.syllabus_id<>sp.syllabus_id;
  ...
  from mark_scheme_groups g join mark_schemes ms on ms.id=g.mark_scheme_id
  join questions q on q.id=ms.question_id join source_papers sp on sp.id=q.source_paper_id
  join syllabi sy on sy.id=sp.syllabus_id and sy.code='9618'
  where q.marks>0 and sp.source_url is not null and ms.scheme_type='any_n_from_m'::scheme_type
    and (
  ...
  from mark_scheme_points p join mark_schemes ms on ms.id=p.mark_scheme_id
  join questions q on q.id=ms.question_id join source_papers sp on sp.id=q.source_paper_id
  join syllabi sy on sy.id=sp.syllabus_id and sy.code='9618'
  ...
  with recursive edges as (
    ...
    union all
    select e.mark_scheme_id,w.start,e.dst,w.path||e.dst from walk w
    ...
    join edges e on e.mark_scheme_id=w.mark_scheme_id and e.src=w.node where e.dst=any(w.path)
  ) select count(*) into cycles from cyc;

  select count(*) into q_missing_answer
  from questions q join source_papers sp on sp.id=q.source_paper_id
  join syllabi sy on sy.id=sp.syllabus_id and sy.code='9618'
  where sp.kind='QP'::paper_kind and sp.source_url is not null and sp.year between 2021 and 2025 and q.marks>0
    and q.stem_md ~* '((your|the) answer (to|from|for) part|use your answer (from|to|for) part|using your answer (from|to|for) part)'
    and not exists(select 1 from question_dependencies qd where qd.question_id=q.id and qd.kind='answer_ref' and qd.strength='required');

  select count(*) into q_missing_practical
  from questions q join source_papers sp on sp.id=q.source_paper_id
  join syllabi sy on sy.id=sp.syllabus_id and sy.code='9618'
  join components c on c.id=q.component_id
  where sp.kind='QP'::paper_kind and sp.source_url is not null and sp.year between 2021 and 2025 and c.number=4 and q.marks>0
  ...
  from question_dependencies qd join questions q on q.id=qd.question_id
  join questions target on target.id=qd.depends_on_id join source_papers sp on sp.id=q.source_paper_id
  join syllabi sy on sy.id=sp.syllabus_id and sy.code='9618'
  where sp.source_url is not null and sp.year between 2021 and 2025 and q.source_paper_id<>target.source_paper_id;

  select count(*) into q_self
  from question_dependencies qd join questions q on q.id=qd.question_id
  join source_papers sp on sp.id=q.source_paper_id
  join syllabi sy on sy.id=sp.syllabus_id and sy.code='9618'
  where sp.source_url is not null and sp.year between 2021 and 2025 and qd.question_id=qd.depends_on_id;
  ...
  with recursive q_edges as (
    ...
    union all
    select e.source_paper_id,w.start,e.dst,w.path||e.dst from q_walk w
    ...
    join q_edges e on e.source_paper_id=w.source_paper_id and e.src=w.node where e.dst=any(w.path)
