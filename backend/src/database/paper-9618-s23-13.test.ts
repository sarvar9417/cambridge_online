...
import { checkLatex } from '../lib/latex.js';
import { TOPICS } from './syllabus-9618-2026.js';
import { PAPER, flattenPaper } from './paper-9618-s23-13.js';
...
describe('9618/13/M/J/23 transcript', () => {
  it('has the seven questions and the expected mark total of 75', () => {
    expect(PAPER.map((node) => node.path)).toEqual(['1', '2', '3', '4', '5', '6', '7']);
    const leaves = flattenPaper(PAPER).filter((item) => !('children' in item)) as SeedLeaf[];
  ...
  it('mark scheme points sum to max marks for all_required schemes', () => {
    for (const leaf of flattenPaper(PAPER).filter((i) => !('children' in i)) as SeedLeaf[]) {
  ...
  it('any_n_from_m schemes have more points than required', () => {
    for (const leaf of flattenPaper(PAPER).filter((i) => !('children' in i)) as SeedLeaf[]) {
      ...
      for (const group of leaf.scheme.groups ?? []) {
        ...
        expect(inGroup.length, `Q${leaf.path} group ${group.label}`).toBeGreaterThan(group.nRequired);
      }
    }
  });
