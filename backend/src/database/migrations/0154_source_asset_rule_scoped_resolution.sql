...
-- other outstanding structure on the same question.  Historical v1 resolution
-- inferred rule coverage only from asset kind, so an image that restored a
-- preceding logo could also resolve an unrelated missing table/layout finding.
-- Future repairs must explicitly declare the fidelity rule(s) proved by each
-- asset.  The SHA/source/idempotency and approval gates remain unchanged.
...
BEGIN
  ...
  IF p_asset ? 'satisfiesRules' THEN
