-- Promote only mark schemes proved by source-safe matcher v6.
--
-- Matcher v6 keeps every matcher-v5 source/canonical/dependency/finding/usage
-- gate and adds only exact unique short-literal proof plus a mark-proved fallback
-- for malformed printed question-path parentheses. No fuzzy or semantic matching.

CREATE OR REPLACE FUNCTION public.ms_source_audit_promote_verified_v6()
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  WITH eligible AS (
    ...
    JOIN public.mark_scheme_source_audits a
     ...
     AND a.evidence->>'matcherVersion'='9618-ms-source-matcher-v6'
...
REVOKE ALL ON FUNCTION public.ms_source_audit_promote_verified_v6() FROM PUBLIC,anon,authenticated;
GRANT EXECUTE ON FUNCTION public.ms_source_audit_promote_verified_v6() TO service_role;
