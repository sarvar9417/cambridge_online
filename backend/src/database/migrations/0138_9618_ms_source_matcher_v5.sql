-- Promote only mark schemes proved by source-safe matcher v5.
--
-- Matcher v5 differs from v4 only by recognizing canonical group labels and
-- exact intra-scheme point-code `requires` references as internal metadata.
-- All official-source, canonical QP, taxonomy, dependency, findings, usage and
-- exact-SHA gates remain unchanged.

CREATE OR REPLACE FUNCTION public.ms_source_audit_promote_verified_v5()
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
REVOKE ALL ON FUNCTION public.ms_source_audit_promote_verified_v5() FROM PUBLIC,anon,authenticated;
GRANT EXECUTE ON FUNCTION public.ms_source_audit_promote_verified_v5() TO service_role;
