-- Reconcile explicit cross-part dependencies for newly ingested source papers.
--
-- Topic/subtopic classification must never make a leaf portable without the
-- source material or prior answer it explicitly consumes. This function handles
-- only deterministic source wording:
--   * "answer ... part (...)" references -> required answer_ref
--   * Paper 4 test/screenshot tasks -> immediately preceding sibling code task
--   * explicit table/diagram/pseudocode references to another part -> text_ref
-- Ambiguous references remain needs_review through validation findings.
...
BEGIN
  ...
  SELECT question_id,depends_on_id,'answer_ref','required',
    ...
    'source-dependency-reconcile-v1',1.0
  FROM resolved
  ...
  -- Paper 4 evidence/test tasks consume the immediately preceding sibling task.
  WITH tests AS (
    ...
    SELECT t.id question_id,p.id depends_on_id,p.display_ref target_ref,
           row_number() OVER(PARTITION BY t.id ORDER BY p.sort_order DESC,p.id) rn
    FROM tests t
  ...
  -- Fail closed: an explicit answer dependency that could not be resolved to a
  -- same-paper source leaf is a release-blocking data problem, not a reason to
  -- guess the target.
  ...
  SELECT
    ...
    jsonb_build_object('displayRef',s.display_ref,'audit','source-dependency-reconcile-v1','year',p_year)
  FROM _source_dep_scope s
  WHERE s.source_text ~
      '(answer (to|from|for) part|use your answer (from|to|for) part|using your answer (from|to|for) part)'
    AND NOT EXISTS (
  ...
  RETURN jsonb_build_object(
    'version','source-dependency-reconcile-v1',
