-- Source-verified, ID-preserving repair for flattened Cambridge QP structures.
--
-- v1 (0110) safely adds one asset. v2 composes that primitive with source-text
-- boundary repair so a fixed question does not keep the old flattened table /
-- diagram text in stem_md. It also supports resolving false-positive fidelity
-- findings when the original source proves that the cue leaked in from another
-- part during extraction.
...
-- Bootstrap every unresolved fidelity target together with all marked leaves on
-- its source paper. The complete ordered leaf list lets the runner recover exact
-- part boundaries from the original QP instead of guessing from corrupted DB text.
