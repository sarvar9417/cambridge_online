-- Fail-closed rule scoping for source-fidelity repair assets.
--
-- A verified source crop may prove one missing structure without proving every
-- other outstanding structure on the same question. Historical resolution
-- inferred rule coverage from asset kind, so one image could resolve unrelated
-- table/layout findings. Future repairs must declare the exact rule(s) proved
-- by each asset. SHA/source/idempotency and approval gates remain unchanged.
...
DECLARE
  ...
  v_satisfies_rules text[] := ARRAY[]::text[];
...
BEGIN
  ...
  v_crop_status := lower(trim(coalesce(
    p_asset->>'cropStatus',CASE WHEN v_storage_path IS NULL THEN 'not_needed' ELSE 'ready' END
  )));
  ...
  IF p_asset ? 'sourcePage' AND nullif(trim(coalesce(p_asset->>'sourcePage','')),'') IS NOT NULL THEN
    ...
    IF v_source_page <= 0 THEN
      RAISE EXCEPTION 'sourcePage must be positive' USING ERRCODE='22023';
    END IF;
  END IF;
  ...
  IF NOT (p_asset ? 'satisfiesRules') THEN
    RAISE EXCEPTION 'repair asset must declare satisfiesRules' USING ERRCODE='22023';
  END IF;
  IF jsonb_typeof(p_asset->'satisfiesRules') <> 'array' THEN
    RAISE EXCEPTION 'satisfiesRules must be an array' USING ERRCODE='22023';
  END IF;
  SELECT coalesce(array_agg(DISTINCT value ORDER BY value),ARRAY[]::text[])
  INTO v_satisfies_rules
  FROM jsonb_array_elements_text(p_asset->'satisfiesRules') AS item(value);
  IF cardinality(v_satisfies_rules)=0 THEN
    RAISE EXCEPTION 'satisfiesRules must name at least one rule' USING ERRCODE='22023';
  END IF;

  IF EXISTS (
    SELECT 1 FROM unnest(v_satisfies_rules) AS r(rule_code)
    ...
    RAISE EXCEPTION 'asset declares an unsupported source-fidelity rule' USING ERRCODE='22023';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'asset declares a rule that is not currently unresolved for the question' USING ERRCODE='22023';
  END IF;

  v_visual := v_kind IN ('image','diagram');
  IF 'source_visual_required_but_missing'=ANY(v_satisfies_rules) AND NOT v_visual THEN
    RAISE EXCEPTION 'a non-visual asset cannot satisfy a visual fidelity rule' USING ERRCODE='22023';
  END IF;
  ...
  ELSE
    ...
    FROM public.question_assets WHERE question_id=p_question_id;
  ...
  SET resolved_at=now(),
      resolution=concat(
        'Resolved by verified source asset ',v_asset_id::text,
        ' from source SHA-256 ',v_actual_sha256,'.'
      )
  WHERE ref_table='questions'
    AND ref_id=p_question_id
    AND resolved_at IS NULL
    AND rule_code=ANY(v_satisfies_rules);
  ...
  SET notes=CASE
        ...
        ELSE concat_ws(E'\n',nullif(notes,''),concat(
          'source-fidelity-repair: asset ',v_asset_id::text,
          ' verified against source SHA-256 ',v_actual_sha256,'.'
        ))
      END,
      updated_at=now()
  ...
  RETURN jsonb_build_object(
    ...
    'sourceVerified',true,
    'declaredRules',to_jsonb(v_satisfies_rules),
...
REVOKE ALL ON FUNCTION public.repair_question_source_asset_v1(uuid,uuid,text,jsonb,boolean)
  FROM PUBLIC,anon,authenticated;
GRANT EXECUTE ON FUNCTION public.repair_question_source_asset_v1(uuid,uuid,text,jsonb,boolean)
  TO service_role;
