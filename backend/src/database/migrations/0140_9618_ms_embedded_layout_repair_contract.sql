-- Second guarded source-repair contract for historical Cambridge 9618 MS points.
--
-- V2 is deliberately narrower than a general rewrite API. It only accepts rows
-- produced from an exact official-source substring after removing deterministic
-- PDF table-layout artefacts (embedded current question path, bounded mark-column
-- integers, or a terminal Published <year> footer). Every write is pinned to the
-- latest matcher-v5 audit, exact MS SHA, page and source-section hash. Repaired
-- schemes remain needs_review until a fresh source audit verifies them.

CREATE OR REPLACE FUNCTION public.apply_ms_source_point_repair_v2(p_manifest jsonb)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  IF p_manifest IS NULL OR jsonb_typeof(p_manifest)<>'object'
     OR p_manifest->>'version'<>'9618-ms-point-source-repair-v2' THEN
    RAISE EXCEPTION 'invalid 9618 MS point repair v2 manifest' USING ERRCODE='22023';
  END IF;
  ...
  IF jsonb_typeof(v_rows)<>'array' OR jsonb_array_length(v_rows)<1 OR jsonb_array_length(v_rows)>80 THEN
    RAISE EXCEPTION 'repair v2 manifest rows must contain 1..80 entries' USING ERRCODE='22023';
  END IF;
  IF EXISTS (
    SELECT 1
    ...
    RAISE EXCEPTION 'duplicate markSchemeId/pointCode in repair v2 manifest' USING ERRCODE='22023';
  END IF;
  ...
  IF NOT FOUND OR v_source_kind<>'MS' OR v_syllabus_code<>'9618'
    ...
    RAISE EXCEPTION 'repair v2 source identity/SHA gate failed' USING ERRCODE='22023';
  END IF;

  -- Validate every row before any mutation so a multi-point manifest is atomic.
  FOR v_row IN SELECT value FROM jsonb_array_elements(v_rows)
  LOOP
    IF jsonb_typeof(v_row)<>'object' THEN
      RAISE EXCEPTION 'invalid repair v2 row' USING ERRCODE='22023';
    END IF;
    ...
    EXCEPTION WHEN OTHERS THEN
      RAISE EXCEPTION 'invalid repair v2 row identifiers' USING ERRCODE='22023';
    END;
    ...
    IF v_point_code='' OR v_old_text='' OR v_new_text='' OR v_mismatch_detail=''
      ...
      RAISE EXCEPTION 'repair v2 row required fields are missing' USING ERRCODE='22023';
    END IF;
    IF v_new_text<>trim(v_new_text) OR length(v_new_text)>8000
       OR length(v_new_text)>=length(v_old_text) THEN
      RAISE EXCEPTION 'repair v2 newText must be a shorter trimmed source-faithful value' USING ERRCODE='22023';
    END IF;
    IF v_proof_mode<>'exact_source_substring_after_embedded_layout_strip_v2' THEN
      RAISE EXCEPTION 'unsupported point repair v2 proof mode' USING ERRCODE='22023';
    END IF;
    ...
    IF NOT FOUND THEN
      RAISE EXCEPTION 'repair v2 point/scheme/source identity mismatch' USING ERRCODE='22023';
    END IF;
    IF v_scheme_status<>'needs_review' OR v_question_status NOT IN ('approved','needs_review')
       OR v_scheme_type='manual_only' THEN
      RAISE EXCEPTION 'repair v2 review-state gate failed' USING ERRCODE='22023';
    END IF;
    IF EXISTS (SELECT 1 FROM public.assignment_questions aq WHERE aq.question_id=v_question_id)
      ...
      RAISE EXCEPTION 'repair v2 question is already in use' USING ERRCODE='22023';
    END IF;
    ...
    IF NOT FOUND OR v_audit_page IS DISTINCT FROM v_source_page
      ...
      RAISE EXCEPTION 'repair v2 matcher-v5 audit proof gate failed' USING ERRCODE='22023';
    END IF;
    ...
    IF v_current_text=v_new_text THEN
      IF NOT EXISTS (
        ...
        RAISE EXCEPTION 'repair v2 current text changed without matching repair history' USING ERRCODE='22023';
      END IF;
    ELSIF v_current_text<>v_old_text THEN
      RAISE EXCEPTION 'repair v2 expectedOldText drifted' USING ERRCODE='22023';
    END IF;
  END LOOP;
  ...
  LOOP
    ...
    IF NOT FOUND THEN
      RAISE EXCEPTION 'repair v2 row changed during apply' USING ERRCODE='40001';
    END IF;
  ...
  RETURN jsonb_build_object(
    'version','9618-ms-point-source-repair-v2',
...
REVOKE ALL ON FUNCTION public.apply_ms_source_point_repair_v2(jsonb) FROM PUBLIC,anon,authenticated;
GRANT EXECUTE ON FUNCTION public.apply_ms_source_point_repair_v2(jsonb) TO service_role;
