...
export async function validateStageManifest(items: SourceStageItem[]) {
  const seen = new Set<string>();
  const validated: Array<SourceStageItem & { component: number; variant: number; absolutePath: string; sha256: string }> = [];
  ...
  for (const item of items) {
    ...
    validated.push({ ...item, component, variant, absolutePath, sha256 });
  }

  return validated;
}

export async function registerStagedSources(pool: Pool, items: SourceStageItem[]): Promise<StagedSource[]> {
      ...
      if (existing.rowCount) {
        ...
        if (currentSha !== item.sha256) {
          ...
          const updated = await client.query(
            `update source_papers set storage_path=$2,sha256=$3,page_count=null where id=$1 returning id`,
            [sourcePaperId, item.absolutePath, item.sha256],
          );
          sourcePaperId = String(updated.rows[0].id);
        } else {
          const updated = await client.query(
            `update source_papers set storage_path=$2 where id=$1 returning id`,
            [sourcePaperId, item.absolutePath],
          );
          sourcePaperId = String(updated.rows[0].id);
        }
      } else {
        const inserted = await client.query(
          `insert into source_papers(syllabus_id,component_id,year,series,variant,kind,storage_path,sha256)
           values($1,$2,$3,$4,$5,$6,$7,$8) returning id`,
          [row.syllabus_id, row.component_id, item.year, item.series, item.variant, item.kind, item.absolutePath, item.sha256],
        );
