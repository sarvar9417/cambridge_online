...
async function sourceHashes(pool:Pool,qpPaperId:string,msPaperId:string){const result=await pool.query(`select id,sha256 from source_papers where id=any($1::uuid[])`,[[qpPaperId,msPaperId]]);const byId=new Map(result.rows.map(row=>[String(row.id),String(row.sha256??'').trim()]));return{qpSha256:byId.get(qpPaperId)??'',msSha256:byId.get(msPaperId)??''}}
