...
export type CrossCheckEvidence={sourceMode:'page_image+text_layer';qpPaperId:string;qpSha256:string;msPaperId:string;msSha256:string;qpPages:number[];msPages:number[]};
...
export async function crossCheckStage(pool:Pool,client:AiClient,input:Artifact,imageLoader:ImageLoader=imageBlock):Promise<Artifact>{
 ...
 const qpPages=pageMap(qp),msPages=ms?pageMap(ms):new Map<number,PageEvidence>(),hashes=ms?await sourceHashes(pool,qp.paperId,ms.paperId):null;
 for(const leaf of questions.filter(question=>question.marks!==null)){
  ...
  if(!hashes?.qpSha256||!hashes.msSha256){crossChecks.push(disagreement(leaf.path,prompt.version,'source.sha256','Exact QP/MS source SHA-256 provenance is unavailable.'));continue}
  ...
  const evidence:CrossCheckEvidence={sourceMode:'page_image+text_layer',qpPaperId:qp.paperId,qpSha256:hashes.qpSha256,msPaperId:ms.paperId,msSha256:hashes.msSha256,qpPages:qEvidence.map(item=>item.page),msPages:mEvidence.map(item=>item.page)};
 ...
 const hasBlocking=crossChecks.some(check=>!check.agrees||check.disagreements.some(item=>item.severity==='error')||check.confidence<.8||!validEvidence(check.evidence));
...
function validEvidence(value:CrossCheckEvidence|undefined){return Boolean(value&&value.sourceMode==='page_image+text_layer'&&value.qpPaperId&&value.msPaperId&&/^[a-f0-9]{64}$/i.test(value.qpSha256)&&/^[a-f0-9]{64}$/i.test(value.msSha256)&&value.qpPages.length&&value.msPages.length)}
...
async function sourceHashes(pool:Pool,qpPaperId:string,msPaperId:string){const result=await pool.query(`select id,sha256 from source_papers where id=any($1::uuid[])`,[[qpPaperId,msPaperId]]);const byId=new Map<string,string>();for(const row of result.rows)byId.set(String(row.id),String(row.sha256??'').trim());return{qpSha256:byId.get(qpPaperId)??'',msSha256:byId.get(msPaperId)??''}}
