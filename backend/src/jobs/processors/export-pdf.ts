...
export function createExportPdfProcessor(pool:Pool){
  return async(job:Job)=>{
    ...
    if(exp.ref_table==='assignments'){
      ...
      title=payload.title||'Cambridge 9618 practice';questions=review.items.map(item=>{const scheme=schemesByQuestion.get(item.portable.leaf.id);return{displayRef:item.freshRef,sourceRef:item.sourceRef,stem:item.portable.leaf.stem,contextBlocks:item.portable.contextBlocks,marks:item.role==='context_only'?0:item.effectiveMarks,answerLines:item.portable.leaf.answerLines,role:item.role,schemeStatus:item.role==='graded'?scheme?.status:undefined,points:item.role==='graded'?(scheme?.points??[]):[]}});assertPaperTotal(questions,Number(review.totalMarks));
    }else{
