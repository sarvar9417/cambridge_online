    ...
    if(exp.ref_table==='assignments'){
      const result=await pool.query(`with items as (
        ...
        select i.*,coalesce(mp.points,'[]'::json) points from items i left join lateral(
          ...
          from canonical_mark_schemes ms join mark_scheme_points msp on msp.mark_scheme_id=ms.id where ms.question_id=i.question_id and ms.status='approved' and i.role='graded')mp on true order by i.sort_order,i.question_id`,[exp.ref_id]);
      ...
      const pointsResult=questionIds.length?await pool.query(`select ms.question_id,ms.status::text scheme_status,json_agg(json_build_object('code',msp.code,'text',msp.text,'marks',msp.marks) order by msp.sort_order) points from canonical_mark_schemes ms join mark_scheme_points msp on msp.mark_scheme_id=ms.id where ms.question_id=any($1::uuid[]) and ms.status in ('approved','needs_review') group by ms.question_id,ms.status`,[questionIds]):{rows:[] as Array<{question_id:string;scheme_status:string;points:ExportQuestion['points']}>};
