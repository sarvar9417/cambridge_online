...
const runner=new JobRunner(pool,{'export-pdf':createExportPdfProcessor(pool)});process.on('SIGTERM',()=>runner.stop());process.on('SIGINT',()=>runner.stop());await runner.start();
